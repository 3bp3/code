...
def add_consts_with_placeholder():
    ...
    af = c1 + c2 + c3
    #sess = tf.Session()
    #print(sess.run(af, feed_dict={c3: data}))
    return af, c3
...
def my_relu(in_value):
    ...
    out_value = tf.cond(tf.less(in_value, 0.0), lambda: tf.constant(0.0), lambda: in_value)
    return out_value

# in_value = tf.placeholder(tf.float32)
# with tf.Session() as sess:
#     print(sess.run(my_relu(in_value), feed_dict={in_value: 3.0} ))
...
def my_perceptron(x):
    ...
    w = tf.get_variable("w", [x], dtype=tf.float32, initializer=tf.ones_initializer,trainable=True)
    out = my_relu(tf.reduce_sum(tf.multiply(i, w)))
    return i, out


# sess = tf.Session()
# i, out = my_perceptron(4)
# init = tf.global_variables_initializer()
# sess.run(init)
# #print(sess.run(w))
# print(sess.run(out, {i: [1,2,3,4]}))
...
def onelayer(X, Y, layersize=10):
    ...
    """
    #create new variable
    #input is a image has 28x28 = 784 pixels,
    #W = [[],[],[].....[]]
    #     |<--784 [] -->|    #   each [] has 10 numbers in it
    # w = tf.get_variable("onew", [784, layersize], dtype=tf.float32, initializer=tf.zeros_initializer())
    # b = tf.get_variable("oneb", [layersize], dtype=tf.float32, initializer=tf.zeros_initializer())

    #tried to change weight to random, but it is very slow to hit 90%
    w = tf.Variable(tf.zeros([784,layersize]))
    b = tf.Variable(tf.zeros([layersize]))

    logits = tf.matmul(X,w) + b
    #be careful in here, X should stay before w
    preds = tf.nn.softmax(logits)

    batch_xentropy = -tf.reduce_sum(Y * tf.log(preds), 1)
    #?????   batch_loss = tf.reduce_mean(tf.reduce_square(logits-Y))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    #
    #w1 = tf.get_variable("twow1", [784, hiddensize], dtype=tf.float32, initializer=tf.random_normal_initializer())
    #b1 = tf.get_variable("twob1", [hiddensize], dtype=tf.float32, initializer=tf.random_normal_initializer())
    #w2 = tf.get_variable("twow2", [hiddensize, outputsize], dtype=tf.float32, initializer=tf.random_normal_initializer())
    #b2 = tf.get_variable("twob2", [outputsize], dtype=tf.float32, initializer=tf.random_normal_initializer())

    #if weight and bias all random, then it takes quite long time to hit 90% accuracy
    #weight2 set to random, bias all set to ones or zeros are ok
    w1 = tf.Variable(tf.zeros([784,hiddensize]))
    b1 = tf.Variable(tf.ones([hiddensize]))
    #
    w2 = tf.Variable(tf.random_normal([hiddensize,outputsize]))
    b2 = tf.Variable(tf.ones([outputsize]))

    #first layer output, using relu activation
    preds1 = tf.nn.relu(tf.matmul(X,w1)+b1)

    #second layer, using softmax activation.
    #Take the output of the first layer as the input X of the second layer
    logits = tf.matmul(preds1, w2) + b2
    preds = tf.nn.softmax(logits)

    '''
    when I set w2 to random, NaN error will come out.
    t happens sometimes during the initial iterations of training that the model might spew out only a single prediction class.
    If out of random chance, the class turned out to be 0 for all the training examples then there can exist a NaN value for Categorical Cross Entropy Loss.
    '''
    batch_xentropy = -tf.reduce_sum(Y * tf.log(preds + 1e-8), 1)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    #caution: not tf.nn.conv2d....
    #choosing relu as the activation function
    conv1 = tf.layers.conv2d(X, filters=convlayer_sizes[0], kernel_size=filter_shape, activation = tf.nn.relu)
    conv2 = tf.layers.conv2d(conv1, filters=convlayer_sizes[1], kernel_size=filter_shape, activation = tf.nn.relu)

    #reshape
    conv2_flat = tf.reshape(conv2, [-1, 784 ])

    #call one layer
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(conv2_flat, Y, 10)
